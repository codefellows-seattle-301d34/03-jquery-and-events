...
articleView.handleAuthorFilter = function() {
  $('#author-filter').on('change', function() {
    ...
    if ($(this).val()) {
      ...
      $('article').hide();
      $(`article[data-author="${$(this).val()}"]`).fadeIn('600');
    } else {
...
articleView.handleCategoryFilter = function() {
  ...
  $('#category-filter').on('change', function() {
    if ($(this).val()) {
      ...
      $('article').hide();
      $(`article[data-category="${$(this).val()}"]`).fadeIn('600');
...
articleView.collapseArticle = function() {
  // DONE: Add an event handler to hide elements beyond the first two in any article body
  $('#articles').delegate('.show-less', 'click', function() {
    event.preventDefault();
    $(this).parent().children('.article-body').children('*:nth-of-type(n+2)').hide();
    $(this).parents('article').children('.read-on').show();
    $(this).remove();
  });
}
